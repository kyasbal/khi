...
* Cloud Shellで実行する場合: Cloud Shellはユーザの権限を用いるため、ログインしているユーザに`IAMと管理` => `IAM`より上記権限を割り当ててください。
* メタデータサーバ環境下で利用する場合: そのメタデータサーバが利用するサービスアカウントに`IAMと管理` => `IAM`より上記権限を割り当ててください。

#### ログ関連の設定

**必須設定**

**必須な設定はありません。デフォルトの状態で機能します。**

**推奨設定**

##### データ書き込み監査ログの設定

KHIは事前設定なく詳細な情報を表示できますが、Kubernetes Engine APIの`DATA_WRITE`監査ログを有効にすることにより、より詳細な表示を提供できます。

1. `IAMと管理` => `監査ログ`
2. `Kubernetes Engine API`の行の左側のチェックボックスをチェック
3. 右側のタブの`データ書き込み`にチェックを入れ保存をクリック

これにより、GKEは複数の種類の監査ログを追加で出力します。

> [!TIP]
> この設定により、 例えばPodやNodeリソースの`.status`フィールドへのパッチリクエストも表示することができるようになります。
> この設定が無効でもPodが削除時の監査ログからその時の最終のコンテナの状態は表示することは可能ですが、Podが削除されない間のコンテナの状態の変更が記録されません。
> もし、トラブルシューティングに詳細なコンテナの情報も必要になる可能性があるのであればこちらの機能を有効にされることをお勧めします。
