...
func TestInspectionTaskGraphInitializer(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			jobMode:     false,
			basePath:    "",
			requestPath: "/api.v1.InspectionTaskGraphService/GetInspectionTaskRegistry",
			wantHit:     true,
		},
		{
			...
			requestPath: "/custom/prefix/api.v1.InspectionTaskGraphService/GetInspectionTaskRegistry",
			wantHit:     true,
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := InspectionTaskGraphInitializer.Init(ctx); err != nil {
				t.Fatalf("InspectionTaskGraphInitializer.Init() failed: %v", err)
			}
