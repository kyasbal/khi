...
func WithDefaultTestInspectionTaskContext(baseContext context.Context) context.Context {
	taskCtx := khictx.WithValue(baseContext, inspectioncontract.InspectionTaskInspectionID, "fake-inspection-id")
	taskCtx = khictx.WithValue(taskCtx, inspectioncontract.InspectionTaskRunID, "fake-run-id")

	taskCtx = khictx.WithValue(taskCtx, inspectioncontract.GlobalSharedMap, typedmap.NewTypedMap())
	taskCtx = khictx.WithValue(taskCtx, inspectioncontract.InspectionSharedMap, typedmap.NewTypedMap())

	ioConfig, err := inspectioncontract.NewIOConfigForTest()
	if err != nil {
		panic("Failed to create test IOConfig: " + err.Error())
	}
	taskCtx = khictx.WithValue(taskCtx, inspectioncontract.CurrentIOConfig, ioConfig)
	taskCtx = khictx.WithValue(taskCtx, inspectioncontract.CurrentHistoryBuilder, history.NewBuilder(ioConfig.TemporaryFolder))
	taskCtx = khictx.WithValue(taskCtx, inspectioncontract.InspectionRunMetadata, generateTestMetadata())
	return taskCtx
}

// RunInspectionTask execute a single task with given context. Use WithDefaultTestInspectionTaskContext to get the context.
func RunInspectionTask[T any](baseContext context.Context, task coretask.Task[T], mode inspection_task_interface.InspectionTaskMode, input map[string]any, taskDependencyValues ...task_test.TaskDependencyValues) (T, *typedmap.ReadonlyTypedMap, error) {
	taskCtx := khictx.WithValue(baseContext, inspectioncontract.InspectionTaskInput, input)
	taskCtx = khictx.WithValue(taskCtx, inspectioncontract.InspectionTaskMode, mode)
...
func RunInspectionTaskWithDependency[T any](baseContext context.Context, mainTask coretask.Task[T], dependencies []coretask.UntypedTask, mode inspection_task_interface.InspectionTaskMode, input map[string]any) (T, *typedmap.ReadonlyTypedMap, error) {
	taskCtx := khictx.WithValue(baseContext, inspectioncontract.InspectionTaskInput, input)
	taskCtx = khictx.WithValue(taskCtx, inspectioncontract.InspectionTaskMode, mode)
