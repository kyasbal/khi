...
func NewQueryGeneratorTask(taskId taskid.TaskImplementationID[[]*log.LogEntity], readableQueryName string, logType enum.LogType, dependencies []taskid.UntypedTaskReference, resourceNamesGenerator DefaultResourceNamesGenerator, generator QueryGeneratorFunc, sampleQuery string) task.Task[[]*log.LogEntity] {
	return inspection_task.NewProgressReportableInspectionTask(taskId, append(
		...
		gcp_taskid.LoggingFilterResourceNameInputGroupTask.GetTaskReference(),
		...
		resourceNames := task.GetTaskResult(ctx, gcp_taskid.LoggingFilterResourceNameInputGroupTask.GetTaskReference())
		loggingFilterResourceNameStore := khictx.MustGetValue(ctx, inspection_task_contextkey.InspectionTaskInput)
		...
		resourceNames.UpdateDefaultResourceNamesForLoggingFilter(taskId.ReferenceIDString(), readableQueryName, defaultResourceNames)
		loggingFilterResourceNameContainer := resourceNames.GetLoggingFilterResourceName(taskId.ReferenceIDString())
		loggingFilterResourceNames := defaultResourceNames
		inputStr, found := loggingFilterResourceNameStore[loggingFilterResourceNameContainer.GetInputID()]
		if found {
			...
			if !hadError {
				loggingFilterResourceNames = resourceNamesList
			}
		}
		...
		for queryIndex, queryString := range queryStrings {
			...
			if taskMode == inspection_task_interface.TaskModeRun {
				...
				queryLogs, queryErr := worker.Query(ctx, readerFactory, loggingFilterResourceNames, progress)
				if queryErr != nil {
