...
var resourceNamesInputKey = typedmap.NewTypedKey[*gcp_types.LoggingFilterResourceNameStore]("query-resource-names")

// LoggingFilterResourceNameInputGroupTask is a task to return the instance of LoggingFilterResourceNameStore from a generated group of form fields.
// Each logging query tasks will register its query on this store and this task generate the group form to allow users to change the target of logging filter.
var LoggingFilterResourceNameInputGroupTask = inspection_task.NewInspectionTask(gcp_taskid.LoggingFilterResourceNameInputGroupTask, []taskid.UntypedTaskReference{}, func(ctx context.Context, taskMode inspection_task_interface.InspectionTaskMode) (*gcp_types.LoggingFilterResourceNameStore, error) {
	sharedMap := khictx.MustGetValue(ctx, inspection_task_contextkey.InspectionSharedMap)
	resourceNamesInput := typedmap.GetOrSetFunc(sharedMap, resourceNamesInputKey, func() *gcp_types.LoggingFilterResourceNameStore {
		return gcp_types.NewLoggingFilterResourceNameStore()
	})
	...
	for _, form := range resourceNamesInput.GetLoggingFilterResourceNames() {
	...
	groupForm := form_metadata.GroupParameterFormField{
		ParameterFormFieldBase: form_metadata.ParameterFormFieldBase{
			Priority:    -1000000, // Priority must be very low to put this advanced field at the bottom of the form.
			ID:          gcp_taskid.LoggingFilterResourceNameInputGroupTask.ReferenceIDString(),
