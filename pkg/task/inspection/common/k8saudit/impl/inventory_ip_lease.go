...
func processEndpointSliceResource(ctx context.Context, group *k8saudit.ResourceManifestLogGroup, leaseHistory k8saudit.IPLeaseHistory) {
	for _, l := range group.Logs {
		if l.ResourceBodyReader == nil {
			continue
		}
		...
		for _, endpointReader := range endpointsReader.Children() {
			...
			for _, ip := range ips {
				switch kind {
				case "pod":
					leaseHistory.TouchResourceLease(ip, l.Log.Timestamp, &k8saudit.ResourceIdentity{
						APIVersion: "core/v1",
						Kind:       "pod",
						Name:       name,
						Namespace:  namespace,
