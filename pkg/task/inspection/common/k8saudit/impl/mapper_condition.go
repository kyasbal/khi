...
func (c *conditionLogToTimelineMapperTaskSetting) ResolveRelatedGroupSets(ctx context.Context, groupedLogs k8saudit.ResourceManifestLogGroupMap) ([]k8saudit.RelatedGroupSet, error) {
...
func (c *conditionLogToTimelineMapperTaskSetting) PreProcessLog(ctx context.Context, passIndex int, event k8saudit.MultiGroupLogEvent, state *conditionLogToTimelineMapperTaskState) (*conditionLogToTimelineMapperTaskState, error) {
	if state == nil {
...
func (c *conditionLogToTimelineMapperTaskSetting) ProcessLog(ctx context.Context, event k8saudit.MultiGroupLogEvent, state *conditionLogToTimelineMapperTaskState) (*khifilev6.TimelineChangeSet, *conditionLogToTimelineMapperTaskState, error) {
	...
	if event.EventType == k8saudit.ChangeEventTypeCreation {
...
var _ k8saudit.ManifestLogToTimelineMapper[*conditionLogToTimelineMapperTaskState] = (*conditionLogToTimelineMapperTaskSetting)(nil)
...
func conditionStateToRevisionState(conditionState string) *pb.RevisionState {
	...
	case "True":
		return k8saudit.RevisionStateConditionTrue
	case "False":
		return k8saudit.RevisionStateConditionFalse
	case "":
		return k8saudit.RevisionStateConditionNoAvailableInfo
	default:
		return k8saudit.RevisionStateConditionUnknown
	}
}
...
func (c *conditionWalker) CheckAndRecord(ctx context.Context, changedTime time.Time, k8sAuditLog *k8saudit.K8sAuditLogFieldSet, condition *model.K8sResourceStatusCondition, cs *khifilev6.TimelineChangeSet) {
	if condition == nil {
...
func MustK8sConditionTimeline(ctx context.Context, ownerPath *khifilev6.TimelinePath, conditionType string) *khifilev6.TimelinePath {
	...
	return builder.TimelineAccumulator.GetPath(ownerPath, khifilev6.PathSegment{
		Name: conditionType,
		Type: k8saudit.TimelineTypeResourceCondition,
	})
}
