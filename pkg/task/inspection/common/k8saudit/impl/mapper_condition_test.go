...
func TestConditionWalker(t *testing.T) {
	...
	k8sFieldSet := &k8saudit.K8sAuditLogFieldSet{
...
`
	...
	t.Run("PreProcessLog and ProcessLog inferred creation with UID tracking", func(t *testing.T) {
		...
		logObj1 := testlog.NewMockLog(
			...
			k8saudit.K8sAuditLogFieldSet{Verb: k8saudit.VerbUpdate, Principal: "user-1", ClusterName: "k8s"},
		)
		bodyYAML1 := `
metadata:
  uid: "uid-1"
...
`
		...
		logObj2 := testlog.NewMockLog(
			...
			k8saudit.K8sAuditLogFieldSet{Verb: k8saudit.VerbPatch, Principal: "user-1", ClusterName: "k8s"},
		)
		bodyYAML2 := `
status:
  conditions:
...
`))
		...
		logObj2 := testlog.NewMockLog(
			...
			k8saudit.K8sAuditLogFieldSet{Verb: k8saudit.VerbPatch, Principal: "user-1", ClusterName: "k8s", IsTruncated: true},
		)
...
`))
		...
		event1 := k8saudit.MultiGroupLogEvent{Log: logObj1, GroupRole: "target", ResourceIdentity: resIdentity, EventType: k8saudit.ChangeEventTypeModification, GroupSet: groupSet}
		event2 := k8saudit.MultiGroupLogEvent{Log: logObj2, GroupRole: "target", ResourceIdentity: resIdentity, EventType: k8saudit.ChangeEventTypeModification, GroupSet: groupSet}
		event3 := k8saudit.MultiGroupLogEvent{Log: logObj3, GroupRole: "target", ResourceIdentity: resIdentity, EventType: k8saudit.ChangeEventTypeModification, GroupSet: groupSet}
...
`))
		...
		logObj2 := testlog.NewMockLog(
			time.Date(2024, 1, 1, 0, 0, 20, 0, time.UTC),
			k8saudit.K8sAuditLogFieldSet{Verb: k8saudit.VerbUpdate, Principal: "user-1", ClusterName: "k8s", IsTruncated: true},
		)
