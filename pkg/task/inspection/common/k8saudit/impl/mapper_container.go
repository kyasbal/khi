...
func (c *containerLogToTimelineMapperTaskSetting) ResolveRelatedGroupSets(ctx context.Context, groupedLogs k8saudit.ResourceManifestLogGroupMap) ([]k8saudit.RelatedGroupSet, error) {
...
func (c *containerLogToTimelineMapperTaskSetting) PreProcessLog(ctx context.Context, passIndex int, event k8saudit.MultiGroupLogEvent, state *containerLogToTimelineMapperTaskState) (*containerLogToTimelineMapperTaskState, error) {
	if state == nil {
...
func (c *containerLogToTimelineMapperTaskSetting) ProcessLog(ctx context.Context, event k8saudit.MultiGroupLogEvent, state *containerLogToTimelineMapperTaskState) (*khifilev6.TimelineChangeSet, *containerLogToTimelineMapperTaskState, error) {
	if state == nil {
		state = &containerLogToTimelineMapperTaskState{
			containerIdentities:   map[string]*containerStatusIdentity{},
			containerStateWalkers: map[string]*containerStateWalker{},
...
var _ k8saudit.ManifestLogToTimelineMapper[*containerLogToTimelineMapperTaskState] = (*containerLogToTimelineMapperTaskSetting)(nil)
...
func (w *containerStateWalker) CheckAndRecord(ctx context.Context, stateReader *structured.NodeReader, cs *khifilev6.TimelineChangeSet, changedTime time.Time, k8sAuditLog *k8saudit.K8sAuditLogFieldSet) {
	...
	if stateReader == nil {
		if w.lastState != "no state" {
			cs.AddRevision(containerPath, &khifilev6.StagingRevision{
				...
				StateType:    k8saudit.RevisionStateContainerStatusNotAvailable,
			})
		...
		if err == nil {
			...
			if err == nil && w.lastState != state {
				cs.AddRevision(containerPath, &khifilev6.StagingRevision{
					...
					ChangedTime:  changedTime,
					StateType:    k8saudit.RevisionStateContainerWaiting,
				})
				w.lastState = state
			}
		}
		...
		if err == nil {
			...
			if err == nil && w.lastStartTime != startTime {
				...
				if err == nil {
					cs.AddRevision(containerPath, &khifilev6.StagingRevision{
						...
						StateType:    k8saudit.RevisionStateContainerStarted,
					})
			...
			if err == nil {
				currentState := "ready"
				revisionState := k8saudit.RevisionStateContainerRunningReady
				if !ready {
					currentState = "not ready"
					revisionState = k8saudit.RevisionStateContainerRunningNonReady
				}
		...
		if err == nil {
			...
			if err == nil && w.lastFinishTime != finishTime {
				...
				if err == nil {
					...
					revState := k8saudit.RevisionStateContainerTerminatedWithSuccess
					if exitCode != 0 {
						revState = k8saudit.RevisionStateContainerTerminatedWithError
					}
					cs.AddRevision(containerPath, &khifilev6.StagingRevision{
						Principal:    k8sAuditLog.Principal,
						VerbType:     k8sAuditLog.Verb,
...
func MustResolveContainerTimelinePath(ctx context.Context, clusterName, namespace, podName, containerName string) *khifilev6.TimelinePath {
	...
	return builder.TimelineAccumulator.GetPath(podPath, khifilev6.PathSegment{
		Name: containerName,
		Type: k8saudit.TimelineTypeContainer,
	})
}
