...
var phaseToState = map[string]*pb.RevisionState{
	"Pending":   k8saudit.RevisionStatePodPhasePending,
	"Running":   k8saudit.RevisionStatePodPhaseRunning,
	"Succeeded": k8saudit.RevisionStatePodPhaseSucceeded,
	"Failed":    k8saudit.RevisionStatePodPhaseFailed,
	"Unknown":   k8saudit.RevisionStatePodPhaseUnknown,
}
...
func (c *podPhaseLogToTimelineMapperTaskSetting) ResolveRelatedGroupSets(ctx context.Context, groupedLogs k8saudit.ResourceManifestLogGroupMap) ([]k8saudit.RelatedGroupSet, error) {
	result := []k8saudit.RelatedGroupSet{}
	processedPods := map[string]struct{}{}

	for _, group := range groupedLogs {
		var podPath string
		var podGroup *k8saudit.ResourceManifestLogGroup
		var bindingGroup *k8saudit.ResourceManifestLogGroup
		...
		if group.Resource.APIVersion == "core/v1" && group.Resource.Kind == "pod" {
			...
			} else if group.Resource.Type() == k8saudit.Subresource && group.Resource.SubresourceName == "binding" {
...
func (c *podPhaseLogToTimelineMapperTaskSetting) PreProcessLog(ctx context.Context, passIndex int, event k8saudit.MultiGroupLogEvent, prevGroupData *podPhaseTaskState) (*podPhaseTaskState, error) {
...
func (c *podPhaseLogToTimelineMapperTaskSetting) ProcessLog(ctx context.Context, event k8saudit.MultiGroupLogEvent, prevGroupData *podPhaseTaskState) (*khifilev6.TimelineChangeSet, *podPhaseTaskState, error) {
	...
	if event.EventType == k8saudit.ChangeEventTypeCreation {
		nonCreationForPod := event.GroupRole == "pod" && k8sFieldSet.Verb != k8saudit.VerbCreate
	...
	case "binding":
		if event.EventType == k8saudit.ChangeEventTypeCreation {
			...
			cs.AddRevision(targetPath, &khifilev6.StagingRevision{
				...
				StateType:    k8saudit.RevisionStatePodPhaseScheduled,
			})
		}
		return cs, prevGroupData, nil
	case "pod":
