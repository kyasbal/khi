			...
			wantRevisions: []wantRevision{
				{
					...
					stateType:   k8saudit.RevisionStatePodPhasePending,
...
`),
				...
				},
				{
					...
					stateType:   k8saudit.RevisionStatePodPhaseSucceeded,
...
`,
			...
			wantRevisions: []wantRevision{
				...
				},
				{
					...
					nodeName:    "node-1",
					changedTime: baseTime.Add(20 * time.Second),
					stateType:   k8saudit.RevisionStatePodPhaseFailed,
					verbType:    k8saudit.VerbUpdate,
					resourceBody: parseYAML(`
metadata:
  uid: uid-1
...
`,
			...
			wantRevisions: []wantRevision{
				...
				},
				{
					...
					nodeName:    "node-1",
					changedTime: baseTime.Add(20 * time.Second),
					stateType:   k8saudit.RevisionStatePodPhaseRunning,
...
`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			podResource := &k8saudit.ResourceIdentity{
				APIVersion: "core/v1",
				Kind:       "pod",
				Namespace:  tc.namespace,
				Name:       tc.podName,
			}
			bindingResource := &k8saudit.ResourceIdentity{
				APIVersion:      "core/v1",
				Kind:            "pod",
				Namespace:       tc.namespace,
				Name:            tc.podName,
			...
			podGroup := &k8saudit.ResourceManifestLogGroup{
				Resource: podResource,
				Logs:     []*k8saudit.ResourceManifestLog{},
			}
			bindingGroup := &k8saudit.ResourceManifestLogGroup{
				Resource: bindingResource,
				Logs:     []*k8saudit.ResourceManifestLog{},
			}

			type stepLogInfo struct {
				manifestLog *k8saudit.ResourceManifestLog
				role        string
			...
			for _, step := range tc.steps {
				...
				mLog := &k8saudit.ResourceManifestLog{
					Log:                logObj,
					ResourceBodyReader: nodeReader,
				}

				var identity *k8saudit.ResourceIdentity
				if step.role == "pod" {
