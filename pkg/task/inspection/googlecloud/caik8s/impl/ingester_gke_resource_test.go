...
func TestGKERawLogTask(t *testing.T) {
	...
	snapshot := &caik8s.GKEResourceSnapshot{
		StartTime: testTime,
	...
	testCases := []struct {
	...
	}{
		{
			...
			snapshots: []*caik8s.GKEResourceSnapshot{snapshot},
			wantCount: 0,
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got, _, err := inspectiontest.RunInspectionTask(ctx, GKELogGrouperTask, inspectioncore.TaskModeRun, map[string]any{},
				tasktest.NewTaskDependencyValuePair(caik8s.GKERawLogTaskID.Ref(), tc.rawLogs),
			)
			if err != nil {
