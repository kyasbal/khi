...
func TestAPIClientFactoryOptionsTask(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			prepareContext: func(ctx context.Context) context.Context {
				opt1 := coreinspection.RunContextOptionArrayElementFromValue(gcpcommon.APIClientFactoryOptionsContextKey, option1)
				opt2 := coreinspection.RunContextOptionArrayElementFromValue(gcpcommon.APIClientFactoryOptionsContextKey, option2)
...
func TestAPICallOptionsInjectorTask(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			desc: "with options",
			prepareContext: func(ctx context.Context) context.Context {
				opt1 := coreinspection.RunContextOptionArrayElementFromValue[googlecloud.CallOptionInjectorOption](gcpcommon.APICallOptionsInjectorContextKey, option1)
				opt2 := coreinspection.RunContextOptionArrayElementFromValue[googlecloud.CallOptionInjectorOption](gcpcommon.APICallOptionsInjectorContextKey, option2)
				ctx, _ = opt1(ctx, inspectioncore.TaskModeRun)
				ctx, _ = opt2(ctx, inspectioncore.TaskModeRun)
				return ctx
			},
