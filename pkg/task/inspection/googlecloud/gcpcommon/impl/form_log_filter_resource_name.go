...
var resourceNamesInputKey = typedmap.NewTypedKey[*gcpcommon.ResourceNamesInput]("query-resource-names")
...
var InputLoggingFilterResourceNameTask = inspectiontaskbase.NewInspectionTask(gcpcommon.InputLoggingFilterResourceNameTaskID, []coretask.Dependency{}, func(ctx context.Context, taskMode inspectioncore.InspectionTaskModeType) (*gcpcommon.ResourceNamesInput, error) {
	...
	groupForm := inspectionmetadata.GroupParameterFormField{
		ParameterFormFieldBase: inspectionmetadata.ParameterFormFieldBase{
			Priority:    -1000000,
			ID:          gcpcommon.InputLoggingFilterResourceNameTaskID.ReferenceIDString(),
...
func getCurrentActiveQueryIDsForResourceName(runner coretask.TaskRunner) []string {
	...
	for _, t := range tasks {
		requestInput, found := typedmap.Get(t.Labels(), gcpcommon.RequestOptionalInputResourceNameTaskLabel)
		if !found {
			continue
		}
