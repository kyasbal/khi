...
func (i *autoscalerLogIngester) ProcessLog(ctx context.Context, l *log.Log) (*khifilev6.LogChangeSet, error) {
	...
	cs.SetLogType(gkeautoscaler.LogTypeAutoscaler)
	cs.SetTimestamp(l.Timestamp)

	autoscalerFieldSet, err := gkeautoscaler.ExtractAutoscalerLog(l.NodeReader)
	if err != nil {
		return nil, err
	}
...
func getDecisionSummary(decision *gkeautoscaler.DecisionLog) string {
...
func getNoDecisionSummary(noDecision *gkeautoscaler.NoDecisionStatusLog) string {
...
func getResultInfoSummary(resultInfo *gkeautoscaler.ResultInfoLog) string {
	statuses := []string{}
...
func mapDecision(ctx context.Context, clusterName string, clusterTimeline *khifilev6.TimelinePath, decision *gkeautoscaler.DecisionLog, cs *khifilev6.TimelineChangeSet) error {
	if decision.ScaleUp != nil {
		scaleUp := decision.ScaleUp
		for _, mig := range scaleUp.IncreasedMigs {
			nodepoolPath := gcpcommon.MustGKENodePoolTimeline(ctx, clusterTimeline, mig.Mig.Nodepool)
			migPath := gkeautoscaler.MustMigTimeline(ctx, nodepoolPath, mig.Mig.Name)
			cs.AddEvent(migPath)
		}
	...
	if decision.ScaleDown != nil {
		...
		for _, nodeToBeRemoved := range scaleDown.NodesToBeRemoved {
			...
			migPath := gkeautoscaler.MustMigTimeline(ctx, nodepoolPath, nodeToBeRemoved.Node.Mig.Name)
			cs.AddEvent(migPath)
	...
	if decision.NodePoolCreated != nil {
		...
		for _, nodepool := range nodePoolCreated.NodePools {
			...
			for _, mig := range nodepool.Migs {
				migPath := gkeautoscaler.MustMigTimeline(ctx, nodepoolPath, mig.Name)
				cs.AddEvent(migPath)
			}
		}
	}
	...
	autoscalerPath := gkeautoscaler.MustAutoscalerTimeline(ctx, clusterTimeline)
	cs.AddEvent(autoscalerPath)
	return nil
}

func mapNoDecision(ctx context.Context, clusterName string, clusterTimeline *khifilev6.TimelinePath, noDecision *gkeautoscaler.NoDecisionStatusLog, cs *khifilev6.TimelineChangeSet) error {
	if noDecision.NoScaleUp != nil {
		...
		for _, groupItem := range noScaleUp.UnhandledPodGroups {
			...
			for _, rejectedMig := range groupItem.RejectedMigs {
				...
				migPath := gkeautoscaler.MustMigTimeline(ctx, nodepoolPath, rejectedMig.Mig.Name)
				cs.AddEvent(migPath)
			}
		}
	}
	...
	if noDecision.NoScaleDown != nil {
		...
		migs := map[string]gkeautoscaler.MIGItem{}
...
func mapResultInfo(ctx context.Context, clusterTimeline *khifilev6.TimelinePath, resultInfo *gkeautoscaler.ResultInfoLog, cs *khifilev6.TimelineChangeSet) error {
	revisionState := gkeautoscaler.RevisionAutoscalerNoError
	if resultInfoHasErrors(resultInfo) {
		revisionState = gkeautoscaler.RevisionAutoscalerHasErrors
	}
...
func resultInfoHasErrors(resultInfo *gkeautoscaler.ResultInfoLog) bool {
	if resultInfo == nil {
		return false
	}
	for _, r := range resultInfo.Results {
