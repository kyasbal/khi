...
func TestProcessPodSandboxIDDiscoveryForLog(t *testing.T) {
	...
	testCases := []struct {
		desc                   string
		inputComponentFieldSet *k8snode.K8sNodeLogCommonFieldSet
		want                   *k8snode.PodSandboxIDInfo
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			finder := patternfinder.NewNaivePatternFinder[*k8snode.PodSandboxIDInfo]()
...
func TestContainerdIDDiscoveryTask(t *testing.T) {
	testCases := []struct {
		desc        string
		messages    []string
		wantPodInfo map[string]k8snode.PodSandboxIDInfo
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			got, _, err := inspectiontest.RunInspectionTask(ctx, PodSandboxIDDiscoveryTask, inspectioncore.TaskModeRun, map[string]any{}, tasktest.NewTaskDependencyValuePair(
				k8snode.ContainerdLogFilterTaskID.Ref(), logs,
			))
			if err != nil {
	...
	testCases := []struct {
	...
	}{
		{
			...
			assert: func(t *testing.T, ctx context.Context, cs *khifilev6.TimelineChangeSet) {
				...
				wantComponentPath := k8snode.MustNodeComponentTimeline(ctx, wantNodePath, "containerd")
