...
type mockGDCVmwareClusterListFetcher struct {
	responsePairs     map[string][]string
	responseWithError bool
}

func (m *mockGDCVmwareClusterListFetcher) GetClusters(ctx context.Context, projectID string) ([]string, error) {
...
var _ googlecloudclustergdcvmware_contract.ClusterListFetcher = (*mockGDCVmwareClusterListFetcher)(nil)

func TestAutocompleteGDCVForVmwareClusterNamesTask(t *testing.T) {
	testCase := []struct {
		desc        string
		...
		},
		{
			...
			clusterList: map[string][]string{
				"foo": {"vm-cluster-1", "vm-cluster-2"},
			},
			projectIDs: []string{"foo", "foo"},
			want: []*googlecloudk8scommon_contract.AutocompleteClusterNameList{
				{
					ClusterNames: []string{"vm-cluster-1", "vm-cluster-2"},
					Error:        "",
				},
				{
					ClusterNames: []string{"vm-cluster-1", "vm-cluster-2"},
	...
	for _, tc := range testCase {
		t.Run(tc.desc, func(t *testing.T) {
			...
			mockClusterListFetcherInput := tasktest.NewTaskDependencyValuePair[googlecloudclustergdcvmware_contract.ClusterListFetcher](googlecloudclustergdcvmware_contract.ClusterListFetcherTaskID.Ref(), &mockGDCVmwareClusterListFetcher{
			...
			for i := 0; i < len(tc.projectIDs); i++ {
				...
				result, _, err := inspectiontest.RunInspectionTask(ctx, AutocompleteGDCVForVMWareClusterNamesTask, inspectioncore_contract.TaskModeDryRun, map[string]any{}, projectIDInput, mockClusterListFetcherInput)
				if err != nil {
					t.Fatalf("failed to run inspection task in loop %d: %v", i, err)
				}

				if diff := cmp.Diff(tc.want[i], result); diff != "" {
					t.Errorf("result of AutocompleteGDCVForVmwareClusterNamesTask mismatch (-want +got):\n%s", diff)
				}
			}
		})
	}
