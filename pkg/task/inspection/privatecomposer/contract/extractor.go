...
var (
	pathDatabaseID                 = structured.CompileFieldPath("resource.labels.database_id")
	...
	pathProtoPayloadPrincipalEmail = structured.CompileFieldPath("protoPayload.authenticationInfo.principalEmail")
)
...
// ExtractCloudSQL extracts CloudSQLFieldSet from a NodeReader.
func ExtractCloudSQL(reader *structured.NodeReader) (CloudSQLFieldSet, error) {
	if mock, ok := structured.GetMock[CloudSQLFieldSet](reader); ok {
		return mock, nil
	}

	rawDatabaseID := reader.ReadStringOrDefault(pathDatabaseID, "")
	...
	} else if reader.Has(pathProtoPayload) {
		methodName := reader.ReadStringOrDefault(pathProtoPayloadMethodName, "")
		principalEmail := reader.ReadStringOrDefault(pathProtoPayloadPrincipalEmail, "")
