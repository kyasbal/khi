...
func TestExtractCloudSQL(t *testing.T) {
	testCases := []struct {
		name      string
		inputYAML string
...
`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			gotFS, err := ExtractCloudSQL(reader)
			if err != nil {
				t.Fatalf("ExtractCloudSQL() returned unexpected error: %v", err)
			}

			if diff := cmp.Diff(tc.wantFS, gotFS); diff != "" {
				t.Errorf("ExtractCloudSQL() mismatch (-want +got):\n%s", diff)
			}
		})
	}

	t.Run("mock node returns mock values", func(t *testing.T) {
		mockFS := CloudSQLFieldSet{
			DatabaseID:  "mock-db",
			LogFileName: "mock.log",
		...
		if diff := cmp.Diff(mockFS, gotFS); diff != "" {
			t.Errorf("ExtractCloudSQL() mismatch (-want +got):\n%s", diff)
		}
	})
}
