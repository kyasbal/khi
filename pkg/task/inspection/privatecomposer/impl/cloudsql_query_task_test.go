...
func TestGenerateCloudSQLStructuredQuery(t *testing.T) {
	testCases := []struct {
		name                   string
...
(LOG_ID("cloudsql.googleapis.com/postgres.log") OR LOG_ID("cloudsql.googleapis.com/postgres-audit.log") OR LOG_ID("cloudsql.googleapis.com/postgres-upgrade.log"))`,
			wantMetricFilters: []string{
				`metric.type = "logging.googleapis.com/log_entry_count" AND resource.type = "cloudsql_database" AND resource.labels.project_id = "my-tenant-project-tp" AND metric.labels.log = one_of("cloudsql.googleapis.com/postgres.log", "cloudsql.googleapis.com/postgres-audit.log", "cloudsql.googleapis.com/postgres-upgrade.log")`,
			},
...
(LOG_ID("cloudsql.googleapis.com/postgres.log") OR LOG_ID("cloudsql.googleapis.com/postgres-audit.log") OR LOG_ID("cloudsql.googleapis.com/postgres-upgrade.log"))`,
			wantMetricFilters: []string{
				`metric.type = "logging.googleapis.com/log_entry_count" AND resource.type = "cloudsql_database" AND resource.labels.project_id = "" AND metric.labels.log = one_of("cloudsql.googleapis.com/postgres.log", "cloudsql.googleapis.com/postgres-audit.log", "cloudsql.googleapis.com/postgres-upgrade.log")`,
			},
			wantSupportMetricsFlag: true,
			wantIncomplete:         true,
		},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			sq := GenerateCloudSQLStructuredQuery(tc.tenantProjectID)
			...
			legacyQuery := GenerateCloudSQLExampleQuery(tc.tenantProjectID)
			if diff := cmp.Diff(gotQuery, legacyQuery); diff != "" {
				t.Errorf("GenerateCloudSQLExampleQuery() mismatch (-want +got):\n%s", diff)
			}
...
func TestCloudSQLListLogEntriesTaskSetting(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if setting.QueryName() != "Cloud Composer Tenant Project Cloud SQL Logs" {
				t.Errorf("QueryName() = %q, want %q", setting.QueryName(), "Cloud Composer Tenant Project Cloud SQL Logs")
			}
		})
	}
}

func TestCloudSQLLogsQueryTask_DryRun(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.tenantProjectID != "" {
				resourceNamesInput.UpdateDefaultResourceNamesForQuery(privatecomposer_contract.CloudSQLLogsQueryTaskID.ReferenceIDString(), []string{"projects/" + tc.tenantProjectID})
			}

			ctx := inspectiontest.WithDefaultTestInspectionTaskContext(t.Context())
			gotLogs, _, err := inspectiontest.RunInspectionTask(ctx, CloudSQLLogsQueryTask, inspectioncore_contract.TaskModeDryRun, map[string]any{},
