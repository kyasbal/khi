...
func (fs *CSMCPFieldSet) Kind() string {
	return "csmcp.khi.google.com/FieldSet"
}

// CSMCPFieldSetReader extracts CSMCPFieldSet from a raw log node reader.
type CSMCPFieldSetReader struct{}

// FieldSetKind returns the Kind string of CSMCPFieldSet.
func (r *CSMCPFieldSetReader) FieldSetKind() string {
	return (&CSMCPFieldSet{}).Kind()
}

// Read parses fields from structured log data.
func (r *CSMCPFieldSetReader) Read(reader *structured.NodeReader) (log.FieldSet, error) {
	...
	if ts, err := reader.ReadTimestamp("jsonPayload.timestamp"); err == nil {
		timestamp = &ts
	}

	instanceId := reader.ReadStringOrDefault("labels.instanceId", "unknown")
...
var _ log.FieldSetReader = (*CSMCPFieldSetReader)(nil)
