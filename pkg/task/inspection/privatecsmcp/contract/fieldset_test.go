...
func TestCSMCPFieldSetReader_Read(t *testing.T) {
	testCases := []struct {
		name    string
		input   string
		want    log.FieldSet
  ...
  timestamp: "2026-07-03T15:46:12Z"`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			r := &CSMCPFieldSetReader{}
			node, err := structured.FromYAML(tc.input)
			if err != nil {
				t.Fatalf("failed to parse yaml: %v", err)
			}
			reader := structured.NewNodeReader(node)
			got, err := r.Read(reader)
			if (err != nil) != tc.wantErr {
				t.Errorf("CSMCPFieldSetReader.Read() error = %v, wantErr %v", err, tc.wantErr)
				return
			}
			if diff := cmp.Diff(tc.want, got); diff != "" {
				t.Errorf("CSMCPFieldSetReader.Read() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
