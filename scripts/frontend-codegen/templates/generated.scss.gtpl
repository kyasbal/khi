...
{{range $key,$type := .LogTypes}}
  $color-{{$type.Label}}: {{$type.LabelBackgroundColor}};
{{end}}
...
{{range $verkeyb,$value := .Verbs}}
  $color-{{$value.CSSSelector | ToLower }}: {{$value.LabelBackgroundColor}};
{{end}}
...
{{range $key,$value := .RevisionStates}}
  $color-revisionstate-{{$value.CSSSelector}}: {{$value.BackgroundColor}};
{{end}}
...
{{range $key,$value := .Severities}}
  $color-severity-{{$value.Label | ToLower}}: {{$value.BackgroundColor}};
{{end}}
{{range $key,$value := .Severities}}
  $color-severity-{{$value.Label | ToLower}}-label: {{$value.LabelColor}};
{{end}}
